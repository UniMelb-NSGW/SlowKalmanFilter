...
@jit(nopython=True)
def propagate_sigma_points(SigmaPoints, tcur, dt):

    PropagatedSigmaPoints = np.empty_like(SigmaPoints)
    for i, x in enumerate(SigmaPoints):
        PropagatedSigmaPoints[i] = rk4_step(f, x, tcur, dt)
...
class KalmanFilterUpdateUKF(object):
    ...
    def Propagate(self, tcur, dt):
        # params_tuple = params['g']
        self.PropagatedSigmaPoints = propagate_sigma_points(self.SigmaPoints, tcur, dt)
